...
{
    DungeonFloor floor = DungeonFloor.Generate(seed);
    Require(floor.Generation.Identity == DungeonFloor.Generate(seed).Generation.Identity, "Same seed must replay.");
...
ExplorationState state = new(new GridPoint(10, 10));
...
state.Advance(ExplorationState.ActionDurationSeconds);
