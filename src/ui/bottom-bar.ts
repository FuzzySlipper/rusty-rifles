...
export function mountBottomBar(root: Element, command: (action: string, fields?: Record<string, unknown>) => void): Readonly<{
  ...
  bar.style.cssText = 'box-sizing:border-box;position:fixed;left:0;right:0;bottom:0;z-index:1;display:grid;grid-template-columns:minmax(200px,220px) minmax(0,1fr) 180px;gap:10px;align-items:stretch;padding:8px 14px;background:#141610f2 url("/product-ui/bar-backdrop.png") center/cover;color:#eee6d5;font:13px/1.35 system-ui;pointer-events:auto;max-height:min(240px,34vh)';
